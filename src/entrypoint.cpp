...
extern "C" EXPORT OodleCompressor *oodle_new(char *data) {
  ...
  auto *on1udpnew_compacted =
      (OodleNetwork1UDP_StateCompacted *)malloc((size_t)statecompacted_size);
  ...
  if (!OodleNetwork1UDP_State_Uncompact_ForVersion(
          comp->state, on1udpnew_compacted, oodle_major_version)) {
    return NULL;
  }
  ...
  OodleNetwork1_Shared_SetWindow(comp->shared, ht_bits, comp->dic,
                                 (OO_S32)dic_size);
...
extern "C" EXPORT bool oodle_decode(OodleCompressor *comp, void *input,
...
extern "C" EXPORT void oodle_destroy(OodleCompressor *comp) {
  free(comp->in_buff);
  free(comp->out_buff);
  free(comp->dic);
  free(comp->state);
